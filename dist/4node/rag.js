...
async function loadFile(filePath, hintExt) {
...
export async function searchKnowledge(query, topK = 5) {
    ...
    if (results.length === 0)
        return '（知识库暂无相关内容）';
    return results.map((doc, i) => `[${i + 1}] (${doc.metadata.sourceFile ?? '未知'}) ${doc.pageContent}`).join('\n\n');
}
/** 上传文件入库 */
export async function ingestFile(filePath, fileName, hintExt) {
...
export async function parseFileToText(filePath, hintExt) {
    const docs = await loadFile(filePath, hintExt);
    return docs.map(d => d.pageContent).join('\n');
}
/** 删除已入库文件 */
export async function removeSource(sourceFile) {
