...
// ── fetchPolicyNode ───────────────────────────────────────────────────────────
export async function fetchPolicyNode(state) {
...
export async function analyzeAndMatchNode(state) {
...
export async function summarizeNode(state) {
    ...
    const summary = suggestions.map((s) => `• **${s.templateName}** / ${s.ruleName}\n  预计加分：${s.estimatedScore} 分\n  理由：${s.reason}`).join('\n\n');
    return { messages: [new AIMessage(`为您匹配到以下加分项：\n\n${summary}`)] };
}
// ── confirmRoute ──────────────────────────────────────────────────────────────
export function confirmRoute(state) {
    const suggestions = state.checkResults
        .map(r => { try {
        return JSON.parse(r);
    }
...
export async function confirmNode(state) {
    ...
    const question = [
        ...
        '',
        ...suggestions.map((s, i) => `${i + 1}. **${s.templateName}** / ${s.ruleName}（预计 ${s.estimatedScore} 分）\n   ${s.reason}`),
    ].join('\n');
...
// ── submitNode ────────────────────────────────────────────────────────────────
export async function submitNode(state) {
    ...
    const proofFileIds = parsed.proofFileIds;
    const proofValues = Array.isArray(parsed.proofValues) ? parsed.proofValues : [];
    if (!state.userInfo) {
    ...
    const submitBody = {
        ...
        proofItems: proofFileIds.map((id, i) => ({
