 ...
 */
export async function analyzeCertificate(certificateText, templates) {
    // 1. RAG 检索相关政策
    ...
    // 3. 构建 Prompt
    const systemMsg = new SystemMessage('你是厦门大学信息学院推免加分审核专家。只输出 JSON 数组，不要任何解释文字，不要 markdown 代码块。');
 ...
 */
export async function generateApplicationRemark(certificateText, templateName, ruleName, estimatedScore) {
    const systemMsg = new SystemMessage('你是厦门大学信息学院推免加分申请助手。根据证明材料内容生成申请备注。只输出备注文本本身，不超过100字，不含任何其他内容。');
