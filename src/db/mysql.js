...
export async function fetchTemplatesFromDb() {
    ...
    if (!p)
        return [];
    try {
        const [templates] = await p.query(`SELECT id, template_name, template_type, score_type, template_max_score
       FROM score_templates WHERE is_active = 1`);
        const result = [];
        for (const t of templates) {
            const [rules] = await p.query(`SELECT id, rule_name, rule_score FROM score_template_rules WHERE template_id = ?`, [t['id']]);
            result.push({
                id: t['id'],
