  ...
  SELECT
    ...
    (
      ...
      WHERE ps.thread_id = checkpoints.thread_id
        ...
        AND ps.channel = '${_langchain_langgraph_checkpoint.TASKS}'
      ORDER BY ps.idx
    ) as pending_sends
  FROM checkpoints
...
var SqliteSaver = class SqliteSaver extends _langchain_langgraph_checkpoint.BaseCheckpointSaver {
	db;
	isSetup;
	withoutCheckpoint;
	withCheckpoint;
...
CREATE TABLE IF NOT EXISTS writes (
  ...
  PRIMARY KEY (thread_id, checkpoint_ns, checkpoint_id, task_id, idx)
	...
	async put(config, checkpoint, metadata) {
		...
		const preparedCheckpoint = (0, _langchain_langgraph_checkpoint.copyCheckpoint)(checkpoint);
	...
	async migratePendingSends(checkpoint, threadId, parentCheckpointId) {
		...
		mutableCheckpoint.channel_values[_langchain_langgraph_checkpoint.TASKS] = await Promise.all(JSON.parse(pending_sends).map(({ type, value }) => this.serde.loadsTyped(type, value)));
		mutableCheckpoint.channel_versions[_langchain_langgraph_checkpoint.TASKS] = Object.keys(checkpoint.channel_versions).length > 0 ? (0, _langchain_langgraph_checkpoint.maxChannelVersion)(...Object.values(checkpoint.channel_versions)) : this.getNextVersion(void 0);
	}
};
